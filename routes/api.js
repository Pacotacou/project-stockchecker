...
module.exports = function (app) {
  ...
  app.route('/api/stock-prices').get(async (req, res) => {
    ...
    if(Array.isArray(stock)){
      ...
      if (!symbol){
        stockData.push({
          rel_likes: Number(firststock.likes.length - secondstock.likes.length)
        });
      } else {
        stockData.push({
          stock: symbol,
      ...
      if (!symbol2){
        ...
        stockData.push({
          stock: symbol2,
          price: Number(latestPrice2),
          rel_likes: Number(secondstock.likes.length - firststock.likes.length)
        });
      }
      res.json({
        stockData
    ...
    res.json({
      stockData: {
        stock: String(symbol),
        price: Number(latestPrice),
        likes: Number(oneStockData.likes.length)
      }
    });
